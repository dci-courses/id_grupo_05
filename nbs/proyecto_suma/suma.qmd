...
(function() {
  ...
  const sketch = (p) => {
    ...
    let lastDetectionTime = 0; // Time when fingers were last detected
    const detectionInterval = 1250; // Interval in milliseconds (1000 ms = 1 second)
    ...
    p.draw = function() {
        ...
        // Check if enough time has passed to detect fingers
        if (p.millis() - lastDetectionTime > detectionInterval) {
            lastDetectionTime = p.millis(); // Update last detection time
            detectFingers(); // Call function to detect fingers
        }
    };
